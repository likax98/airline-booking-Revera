...
describe("helpers", () => {
  ...
  describe("getCalendarWarningMessage", () => {
    it("returns origin/destination warning if either is missing", () => {
      const cases = [
        { origin: "", destination: sharedParams.origin },
        { origin: sharedParams.destination, destination: "" },
      ];

      cases.forEach((params) => {
        expect(getCalendarWarningMessage(params)).toBe(
          MESSAGES.SELECT_ORIGIN_AND_DESTINATION
        );
      });
    });
  ...
  describe("getDefaultValuesFromSearchParams", () => {
    ...
    it("uses provided values from search params", () => {
      ...
      expect(result.toDate).toEqual(expected.toDate);
    });
